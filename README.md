...
* Initalize the `$OVPN_DATA` container that will hold the configuration files and certificates
        ...
        docker run --volumes-from $OVPN_DATA --rm kylemanna/openvpn ovpn_genconfig -u udp://`ifconfig eth0 | grep "inet " | awk '{print $2}'`:$OVPN_PORT
        docker run --volumes-from $OVPN_DATA --rm -it kylemanna/openvpn ovpn_initpki

* Start OpenVPN server process

        docker run --volumes-from $OVPN_DATA -d -p $OVPN_PORT:$OVPN_PORT/udp --privileged kylemanna/openvpn

* Generate a client certificate without a passphrase

        docker run --volumes-from $OVPN_DATA --rm -it kylemanna/openvpn easyrsa build-client-full LEGITVPN nopass

* Retrieve the client configuration with embedded certificates

        docker run --volumes-from $OVPN_DATA --rm kylemanna/openvpn ovpn_getclient LEGITVPN > LEGITVPN.ovpn
